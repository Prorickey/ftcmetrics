  ...
  try {
    ...
    if (!code)
      return Response.json(
        { error: 'Missing code from query params' },
        { status: 401 }
      );
    ...
    if (!email)
      return Response.json(
        { error: 'Could not retrieve verified account' },
        { status: 401 }
      );
    ...
    const response = NextResponse.redirect(
      `http://localhost:3000/account/create?email=${encodeURIComponent(email)}&identity=google`,
      { status: 302 }
    );
